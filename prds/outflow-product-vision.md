...
- **Browser compatibility evidence:** The public guest tracker has a direct desktop Chromium, Firefox, and WebKit guest compatibility gate for shell rendering, subscription CRUD and recurrence, calendar and timeline behavior, CSV and iCalendar downloads, backup/restore, and local ledger isolation. Desktop and mobile Chromium remain the broader PWA, notification, reflow, and automated accessibility profiles.
- **Operator control-plane evidence:** A versioned, read-only GitHub environment readiness contract binds staging plus macOS, iOS, and Android production workflows to exact setting-name inventories, `main`-only deployment policies, and required reviewers without requesting secret values. External environments and credentials remain operator-owned and unconfigured.
