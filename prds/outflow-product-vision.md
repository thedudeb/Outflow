...
- **Next architecture gate:** The repository now enforces the six-function JWT inventory, validates hosted/local/legacy Supabase key modes without exposing values, type-checks plus runtime-tests every Edge Function boundary on pinned Deno, and provides protected, manually dispatched staging workflows for public CORS/rejection boundaries and an authenticated synthetic account-plane matrix with scoped reports and deterministic cleanup. Provision a non-production Supabase/Resend/Stripe test environment using the service runbook, apply all migrations and functions, pass both repository staging workflows, schedule the reminder worker from Supabase Cron/Vault, then prove invitation email delivery, reminder delivery/retry, two-browser Realtime transport, signed payment/refund fulfillment and restore, and production calendar-client behavior before enabling accounts or purchases publicly.
