...
- **Shipped locally:** Guest ledger, subscription CRUD, pause/resume, forecasts, billing calendar, trials, categories, tags, multiple currencies without implied conversion, CSV import/export, configurable device-alert lead times, paused-alert opt-in, versioned active-ledger backup/restore, isolated personal/household/team ledger switching, local change attribution, recurring iCalendar export, installable web metadata, update handling, verified offline relaunch, and persisted dismissible account prompts after meaningful activity, backup, sharing, and installation moments.
