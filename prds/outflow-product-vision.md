...
- **Next architecture gate:** The repository now enforces the six-function JWT inventory, validates hosted/local/legacy Supabase key modes without exposing values, type-checks plus runtime-tests every Edge Function boundary on pinned Deno, and provides four protected, manually dispatched staging workflows: public CORS/rejection boundaries; an authenticated synthetic account plane covering hosted RLS, migration, roles, revision writes, initial cross-client Realtime delivery, explicit disconnect, missed-write conflict, authoritative catch-up, post-reconnect delivery, calendar publication/HTTP lifecycle, revocation, and deletion; a Stripe-test-only billing plane covering canonical one-time Checkout configuration, signed fulfillment/refund idempotency, second-session restore, revocation, and deterministic cleanup without making a card charge; and a Resend-test-address messaging plane covering provider-backed invitation delivery and recipient acceptance plus reminder delivery, deterministic retry, idempotency, pause scope, opt-out, refund suspension, and cascade cleanup. Provision a non-production Supabase/Resend/Stripe test environment using the service runbook, apply all migrations and functions, pass all repository staging workflows, schedule the reminder worker from Supabase Cron/Vault, then prove browser-visible Realtime disconnect/reconnect and stale-edit states against the hosted project, actual Stripe-hosted payment plus provider-originated webhook delivery, actual reminder failure and bounce handling, human-mailbox provider breadth, and third-party calendar-client behavior before enabling accounts or purchases publicly.
