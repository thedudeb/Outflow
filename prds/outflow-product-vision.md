...
- **Next architecture gate:** The repository now enforces the six-function JWT inventory, validates hosted/local/legacy Supabase key modes without exposing values, type-checks plus runtime-tests every Edge Function boundary on pinned Deno, and provides a protected, manually dispatched staging workflow for exact CORS and rejected JWT/signature/cron/token requests with a scoped commit and migration report. Provision a non-production Supabase/Resend/Stripe test environment using the service runbook, apply all migrations and functions, pass the public boundary workflow, schedule the reminder worker from Supabase Cron/Vault, then prove cross-user RLS isolation, guest migration, invitation delivery/acceptance, reminder delivery/retry, two-browser conflict/Realtime behavior, signed payment/refund fulfillment, restore, and deletion before enabling accounts or purchases publicly.
