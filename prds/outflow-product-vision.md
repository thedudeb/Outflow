...
- **Next architecture gate:** Provision a non-production Supabase/Resend/Stripe test environment, apply all migrations and functions, then prove cross-user RLS isolation, guest migration, invitation delivery/acceptance, two-browser conflict/Realtime behavior, signed payment/refund fulfillment, restore, and deletion before enabling accounts or purchases publicly.
