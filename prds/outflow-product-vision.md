...
- Trial end dates and their expected first paid charges, flexible categories and tags, reusable built-in and user-saved subscription packs, mixed-currency records, reviewed CSV import/export, and bounded custom/multiple device-alert lead times are available in the local subscription list.
...
- **Integration evidence:** Pro accounts can create expiring, revocable personal access tokens for a documented HTTP API and local stdio MCP server. Tokens are shown once and stored only as hashes; reads and writes share cloud membership, editor, entitlement, maintenance, validation, request-size, and rate-limit boundaries. Five MCP tools cover list discovery plus subscription read, create, update/pause, and delete without exposing device-only guest data.
...
4. A user can save selected subscriptions as a reusable pack and apply it to another list after entering fresh billing dates.
...
1. A Pro user can create an expiring personal access token, see when it was last used, and revoke it immediately.
2. API and MCP clients can read only synced lists visible to that account and can change subscriptions only where the account has current write access.
3. Device-only guest records, plaintext credentials, account emails, payment data, and provider metadata must not appear in integration responses.
4. API and MCP writes must use the same subscription validation, entitlement, collaboration, maintenance, and synchronization revision boundaries as first-party clients.
...
- Pro must unlock cross-device sync, shared subscription lists, reviewed CSV import, external calendar integration, API/MCP access, and advanced alert rules. Canonical CSV export and all supported currencies remain free for data ownership and local use.
