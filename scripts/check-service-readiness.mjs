...
export function validateServiceEnvironment(env, { allowLocal = false } = {}) {
  ...
  if (values.OUTFLOW_OPERATIONS_SECRET && (values.OUTFLOW_OPERATIONS_SECRET.length < 32 || /\s/.test(values.OUTFLOW_OPERATIONS_SECRET) || new Set(values.OUTFLOW_OPERATIONS_SECRET).size < 12)) {
    errors.push("OUTFLOW_OPERATIONS_SECRET: expected at least 32 high-entropy, whitespace-free characters.");
  }
  if (values.OUTFLOW_CRON_SECRET && values.OUTFLOW_OPERATIONS_SECRET && values.OUTFLOW_CRON_SECRET === values.OUTFLOW_OPERATIONS_SECRET) {
