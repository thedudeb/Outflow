...
test("household ledgers isolate totals, records, attribution, and active state", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Manage Home subscriptions", exact: true })).toBeVisible();
  ...
  await expect(page.getByRole("button", { name: "Manage Personal subscriptions", exact: true })).toBeVisible();
  await expect(monthlyOutflowCard(page)).toContainText("$39.47");
  await expect(page.getByRole("article")).toHaveCount(5);
