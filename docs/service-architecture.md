...
10. Pass the protected account-plane disconnect, stale-conflict, authoritative catch-up, and post-reconnect delivery sequence, then repeat the fixture-backed two-browser visible-state matrix against the provisioned project before describing synchronization as available publicly.
