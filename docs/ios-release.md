...
6. Set `https://thedudeb.github.io/Outflow/?view=privacy` as the App Store Connect Privacy Policy URL, reconcile its version with the exact candidate's manifest, webview traffic, providers, and app-level privacy answers, then complete signed upgrade, real-device, notification, VoiceOver, text-scaling, configured-service, TestFlight, and staged-release checks before promotion.
