...
**Status:** Dialog and motion baseline implemented; full release audit pending
...
Before describing the web product as accessibility-audited, run automated WCAG checks and manual screen-reader testing with VoiceOver/Safari and NVDA/Firefox or NVDA/Chrome. Cover dashboard reading order, form validation announcements, dynamic sync and payment status announcements, calendar navigation, color contrast, browser zoom, and notification-permission flows in addition to the dialog matrix above.
