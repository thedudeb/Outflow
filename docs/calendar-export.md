...
Outflow exports a standards-based iCalendar (`.ics`) file containing one recurring all-day event per selected subscription. The file is generated locally and does not transmit subscription-list data to Outflow or a calendar provider.

## Event Mapping

| Outflow field | iCalendar field |
| --- | --- |
| Subscription and internal list IDs | `UID` |
...
| Cycle and list identity | `DESCRIPTION` |
...
The event UID is stable for the life of a subscription and includes both its subscription ID and internal list ID:
...
Calendar exports include only subscription name, amount, currency, cycle, category, next billing date, status, and list identity. They exclude tags, trial dates, reminder rules, account credentials, browser permissions, payment details, and bank information.
...
- Default downloads include active subscriptions only and preserve Outflow, list, category, amount, recurrence, privacy, transparency, and free/busy identity.
...
- Export filenames and calendar names remain list-specific and recognizable as Outflow data.
