# Outflow Subscription List Backup Contract

Outflow's full-list backup is the portable boundary between today's active list on this device and future account synchronization. It preserves that list's subscription records, identity metadata, and application-level alert settings without including other lists on this device, browser permissions, or notification history.

Signed-in users can separately download their complete user-visible cloud footprint through the [Account Data Export Contract](account-data-export.md). Account export is an archive and data-control surface; this backup format remains the reviewed restore boundary.
...
- **Replace all** replaces the active list's subscriptions, name, and application-level alert settings with the validated backup. Its workspace slot keeps its existing ID, type, and storage mode so another list cannot be overwritten or impersonated by an imported file.
...
1. Authenticate the user without changing or deleting the list on this device.
...
Account deletion must remove account-held copies without deleting an independent list on this device unless the user separately confirms that action.
...
- Downloads contain the complete versioned active-list envelope, normalized records, alert settings, and unique stable identifiers without browser permission or notification-history data.
- Merge skips both identifier duplicates and normalized content duplicates, adds only new records, preserves the active list and alert settings, and survives a reload.
- Replace restores subscriptions, list name, and application-level alert settings while retaining the current device slot's identifier, type, and storage mode.
- A requested device-alert state is not restored when the browser has not granted notification permission.
- Duplicate identifiers and unsupported schema versions produce connected accessible errors and leave the active list unchanged.
