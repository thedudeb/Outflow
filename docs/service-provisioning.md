...
The local account-service suite performs the visible Realtime refresh, stale-edit, and reconnect flow through two isolated browser contexts and a Phoenix-protocol fixture. The account-plane workflow separately proves the deployed publication, authorization, missed-write conflict, authoritative catch-up, and post-reconnect transport sequence through two authenticated service clients. Repeat the browser UI matrix against the provisioned project because neither proof alone verifies the complete browser-to-hosted-service path.
...
- Browser-visible stale-edit protection plus `offline`, `conflict`, `stale`, and `synced` transitions against provisioned Realtime; the account-plane workflow separately proves the hosted two-client disconnect, catch-up, and post-reconnect update sequence.
