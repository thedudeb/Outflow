...
- Reminder timezone boundaries, concurrent-worker behavior, actual provider API failure, and operational alert delivery beyond the messaging-plane Cron/Vault registration, recent scheduler run, delivery, retry, idempotency, pause-scope, opt-out, refund, and signed bounce/complaint checks.
