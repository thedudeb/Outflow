...
After the staging worker and hourly scheduler are proven, set these repository-level Actions variables:

| Repository Actions variable | Value |
| --- | --- |
| `OUTFLOW_OPERATIONS_ENABLED` | Literal `true` only after hourly monitoring is ready |
| `OUTFLOW_OPERATIONS_ASSIGNEE` | One GitHub username with permission to be assigned repository issues |

The enable flag intentionally lives outside the `staging` environment because GitHub evaluates the scheduled job gate before granting environment access. Keep the dedicated operations secret in the protected `staging` environment and ensure it differs from `OUTFLOW_CRON_SECRET`.
...
After the messaging plane passes, configure `OUTFLOW_OPERATIONS_ASSIGNEE`, set the repository Actions variable `OUTFLOW_OPERATIONS_ENABLED=true`, and manually dispatch **Reminder Operations** once. The same protected job then runs at minute 23 of every hour, calls the worker's dedicated aggregate-health action, writes fixed aggregate evidence to its summary, emits bounded warnings for smaller failure/retry/suppression movement, and fails on critical codes. It receives no Supabase publishable/service key, provider, webhook, cron, database-password, or deployment credential.

On failure, the job uses its repository-scoped `issues: write` permission to open or update one privacy-bounded issue and assign the configured operator before re-applying the failed health result. On recovery, it comments on and closes that same incident. Inject an isolated staging failure and prove GitHub notifies the assignee, then restore health and prove automatic resolution before treating this as a production alert path. See [Reminder Operations](reminder-operations.md).

After the account plane passes, manually dispatch **Staging Browser Sync** from `main`. It provisions a fresh owner/editor pair and shared Pro ledger for desktop Chromium, mobile Chromium, desktop Firefox, and desktop WebKit, then runs the deployed Outflow UI in two isolated contexts per profile. Each profile first aborts one owner write before Supabase receives it, verifies the exact bounded browser recovery record without credentials or identity data, reloads, replays the same immutable operation, observes one server application, and requires local cleanup. It then verifies a hosted Realtime refresh, stale-edit preservation and recovery, server-side conflict rejection, visible Realtime disconnect, authoritative reconnect catch-up, and a final synchronized state. The harness suppresses one incoming database frame solely to create a deterministic stale revision, and closes only the tested browser's Realtime WebSocket to create a deterministic transport interruption; every non-aborted read and write uses the deployed app and hosted project.

The browser workflow disables screenshots, traces, videos, downloads, retries, and parallel workers. Its fixed summary contains only the browser profile, deployment metadata, and named checks. It excludes synthetic identities, sessions, row identifiers, operation identifiers, payloads, and Realtime frames. A pass proves the pinned Chromium, Firefox, and WebKit browser-to-hosted-service paths, including foreground reload recovery for one ambiguous write; it does not prove background synchronization while the app is closed, prolonged offline multi-edit merging, branded Safari, assistive-technology compatibility, or production availability. See [Browser Compatibility Contract](browser-compatibility.md).
