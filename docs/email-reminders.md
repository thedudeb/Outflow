...
Deploy `send-due-reminders` and `resend-webhook` with JWT verification disabled only after setting all values in `supabase/functions/.env.example`. Set `OUTFLOW_DEPLOYMENT_COMMIT` to the exact lowercase 40-character Git SHA being deployed; the protected workflow sets it from `github.sha` and rejects any worker response that does not attest that value. The worker performs its own constant-time bearer-secret check. The webhook verifies `svix-id`, `svix-timestamp`, and `svix-signature` against the server-only `RESEND_WEBHOOK_SECRET` before parsing JSON.
