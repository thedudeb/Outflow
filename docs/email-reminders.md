...
Deploy `send-due-reminders` and `resend-webhook` with JWT verification disabled only after setting all values in `supabase/functions/.env.example`. Set `OUTFLOW_DEPLOYMENT_COMMIT` to the exact lowercase 40-character Git SHA being deployed; the protected workflow sets it from `github.sha` and rejects any worker response that does not attest that value. The worker performs constant-time checks for distinct cron and operations bearer secrets. The webhook verifies `svix-id`, `svix-timestamp`, and `svix-signature` against the server-only `RESEND_WEBHOOK_SECRET` before parsing JSON.
...
The protected workflow automates exact-commit migration/function deployment and attestation, the exact Cron/Vault registration and recent-run check, concurrent-worker isolation, active and paused scope, idempotency, opt-out, refund, provider delivery, planned provider-API failure/retry, signed provider-bounce and complaint suppression, explicit bounce recovery, deterministic internal retry, and aggregate operational health. Timezone-provider breadth, unplanned provider outages, mailbox-provider breadth, and delivery through a configured operator-notification path remain manual release checks. None of these hosted claims is complete until the external staging entries are configured and the protected workflow passes.
