...
Outflow Pro accounts can publish one private iCalendar feed per synced list. The feed is a pull-based subscription: calendar clients fetch the same URL again and receive current synced subscription versions without the user repeatedly importing a downloaded file.

## Security Contract

- Publishing requires an authenticated account with active lifetime Pro and current membership in the target list.
...
- Every fetch rechecks the publisher's active Pro entitlement and list membership. Membership removal also deletes that member's feed immediately, so a later re-invitation cannot reactivate the old URL. Refunds, revocation, feed revocation, list deletion, and account deletion all stop access without waiting for a client refresh.
...
`verify_jwt` is disabled only for this function because external calendar clients do not have an Outflow session. The random token is verified in the handler before any subscription-list data is returned. Invalid, rotated, suspended, and revoked tokens receive the same not-found response.
...
A downloaded `.ics` file remains available for lists on this device and synced lists without publishing anything. Hosted feeds are never created automatically.
...
4. Verify conditional `GET`, `HEAD`, empty feeds, paused scope, rotation, revocation, refund, membership removal, list deletion, and account deletion.
5. Confirm malformed and old tokens reveal no list metadata and that monitoring redacts the `token` query parameter.
